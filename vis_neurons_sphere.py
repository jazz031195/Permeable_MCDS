...
def draw_neurons(neuron_ids):
    ...
    for neuron_id in neuron_ids:
        df = pd.read_csv(f"/home/localadmin/Documents/MCDC_perm_jas/Permeable_MCDS/results/packing/neuron_{neuron_id}.csv")
    ...
    with open(neuron_file) as f:
        ...
        for i in range(len(lines)):
            ...
            if (i + 1 < len(lines)):
                ...
                if ("Soma" in lines[i+1].split(' ')[0]) and not ("Neuron" in lines[i].split(' ')[0]):
                    if len(df.index) > 1:
                        df.to_csv(f"/home/localadmin/Documents/MCDC_perm_jas/Permeable_MCDS/results/packing/neuron_{neuron_id}.csv")
                        df = pd.DataFrame(columns=["neuron_id", "x", "y", "z", "r"])
...
neuron_file = 'results/packing/_rep_22_neurons_list.txt'
