            ...
            for filename in os.listdir(DWI_folder / overlap / neuron):
                ...
                if "DWI_img" in filename:
                    ...
                    for i in range(nb_G):
                        ...
                        d = {'loc': "intra", 'N': N, 'T': T, 'Sb/So': mean, 'b [ms/um²]': b_lab, 'neuron': neuron, 'overlap': int(overlap.split('_')[-1])}
...
fig, ax = plt.subplots(1, 1)
sns.violinplot(data=df_dwi[df_dwi['b [ms/um²]'] > 0], x='b [ms/um²]', y='Sb/So', hue='overlap', ax=ax)
