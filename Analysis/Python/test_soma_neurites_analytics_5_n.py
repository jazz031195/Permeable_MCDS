...
def get_dwi_array(dwi_path):
    ...
    print(np.fromfile(dwi_path, dtype="float32"))
...
def create_df(DWI_folder):
    ...
    for neuron in os.listdir(DWI_folder):
        print(neuron)
        f    = open(DWI_folder / neuron / "params.json")
        data = json.load(f)
        ...
        for filename in os.listdir(DWI_folder / neuron):
            ...
            if "DWI_img" in filename:
                ...
                for i in range(nb_G):
                    ...
                    d = {'loc': "intra", 'N': N, 'T': T, 'Sb/So': mean, 'b [ms/um²]': b_lab, 'neuron': neuron}
...
DWI_folder = Path("/home/localadmin/Documents/MCDC_perm_jas/Permeable_MCDS/results/funnel/overlap_4/")
...
cases = ['soma', 'dendrites', 'soma dendrites', 'soma dendrites ex', 'soma dendrites ex T']
    ...
    for n_i, n in enumerate(N_labels):
        ...
        for c in cases:
            ...
            if plot:
                fmt_ = '.'
                c_tmp = c
                if "soma dendrites ex" in c_tmp or "soma dendrites" in c_tmp:
                    
                    if "soma dendrites ex" in c_tmp:
                        fmt_ = '2'
